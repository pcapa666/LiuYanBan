  ...
  return common_vendor.e({
    a: common_vendor.f($data.onlist, (item, k0, i0) => {
      return {
        ...
        f: common_vendor.o(($event) => $options.onDelete(item._id), item._id),
        g: common_vendor.t(item.content),
        h: common_vendor.t($options.getMessageTime(item.CurrentTime)),
        i: common_vendor.o(($event) => $options.onCollect(item, $event), item._id),
        j: $options.getOnCollect(item),
        k: common_vendor.o(($event) => $options.onCollect(item, $event), item._id),
        l: !$options.getOnCollect(item),
        m: common_vendor.t(item.good.num),
        n: common_vendor.o(($event) => $options.onGood(item, $event), item._id),
        o: $options.getOnGood(item),
        p: common_vendor.t(item.good.num),
        q: common_vendor.o(($event) => $options.onGood(item, $event), item._id),
        r: !$options.getOnGood(item),
        s: common_vendor.t($options.getCommentNum(item)),
        t: common_vendor.o(($event) => $options.toMessage(item), item._id),
        v: item._id
      };
    }),
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__file", "D:/Not_Happy/Yang/pages/myMessage/myMessage.vue"]]);
